...
pub const Container = enum {
    ...
    fn parseGzipHeader(reader: *std.Io.Reader) !void {
        const magic1 = try reader.takeInt(u8, .little);
        const magic2 = try reader.takeInt(u8, .little);
        const method = try reader.takeInt(u8, .little);
    ...
    fn parseZlibHeader(reader: *std.Io.Reader) !void {
        const cm = try reader.takeInt(u4, .little);
        const cinfo = try reader.takeInt(u4, .little);
    ...
    pub fn parseFooter(comptime wrap: Container, hasher: *Hasher(wrap), reader: *std.Io.Reader) !void {
        switch (wrap) {
            ...
            .zlib => {
                ...
                if (try reader.takeInt(u32, .little) != chksum) return error.WrongZlibChecksum;
            },
            .raw => {},
        }
    }
