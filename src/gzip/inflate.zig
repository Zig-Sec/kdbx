        ...
        fn step(self: *Self) !void {
            switch (self.state) {
                ...
                .protocol_footer => {
                    ...
                    try container.parseFooter(&self.hasher, self.bits.forward_reader);
                    self.state = .end;
                },
                .end => {},
            }
...
test "decompress" {
    ...
    for (cases) |c| {
        ...
        var al = std.Io.Writer.Allocating.init(testing.allocator);
        defer al.deinit();

        try decompress(.raw, &fb, &al.writer);
        try testing.expectEqualStrings(c.out, al.written());
    }
}
...
test "gzip decompress" {
    ...
    for (cases) |c| {
        ...
        try decompress(.gzip, &fb, &al.writer);
        try testing.expectEqualStrings(c.out, al.written());
    }
}
...
test "zlib decompress" {
    ...
    for (cases) |c| {
        ...
        try decompress(.zlib, &fb, &al.writer);
        try testing.expectEqualStrings(c.out, al.written());
    }
}
...
test "fuzzing tests" {
    ...
    inline for (cases, 0..) |c, case_no| {
        ...
        if (c.err) |expected_err| {
            try testing.expectError(expected_err, decompress(.raw, &in, &out.writer));
        } else {
            try decompress(.raw, &in, &out.writer);
            try testing.expectEqualStrings(c.out, out.written());
        }
    }
}
...
test "bug 18966" {
    ...
    var fb = std.Io.Reader.fixed(input);
    var al = std.Io.Writer.Allocating.init(testing.allocator);
    defer al.deinit();

    try decompress(.gzip, &fb, &al.writer);
    try testing.expectEqualStrings(expect, al.written());
}
...
test "bug 19895" {
    ...
    var decomp = decompressor(.raw, &in);
