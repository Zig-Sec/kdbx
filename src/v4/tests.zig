...
// TODO: the mac is missing (don't know why the test always passed until now)
...
//
//    var fbs = std.Io.Reader.fixed(s);
//
//    const header = try Header.readAlloc(&fbs, std.testing.allocator);
//    defer header.deinit();
//
...
//    var fbs = std.Io.Reader.fixed(s);
//
//    var header = try Header.readAlloc(&fbs, std.testing.allocator);
//    defer header.deinit();
//
...
test "verify kdbx4 header mac (positive test)" {
    var fbs = std.Io.Reader.fixed(db);
...
test "the decryption of a kdbx4 file #1" {
    ...
    var body_xml = try body.getXml(std.testing.allocator);
...
test "serialize entry #1" {
    ...
    var arr: std.ArrayListUnmanaged(KeyValue) = .empty;

    try arr.append(allocator, KeyValue{
...
test "serialize entry #2" {
    ...
    try group.entries.append(allocator, entry1);
    try group.entries.append(allocator, entry2);
...
test "serialize Meta #1" {
    ...
    var icons: std.ArrayListUnmanaged(Icon) = .empty;
    try icons.append(allocator, .{
    ...
    var data: std.ArrayListUnmanaged(KeyValue) = .empty;
...
test "create new database #1" {
    ...
    //    // ------------------------------------
    //
    var fbs = std.Io.Reader.fixed(raw.written());
    const reader = &fbs;
